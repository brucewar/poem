...
Server.prototype.responseHandler = function (dataBuf) {
  ...
  while (response) {
    if (response.header.opcode == protocol.opcode.SASL_LIST_MECHS) {
      this.saslAuth();
    } else if (response.header.status == protocol.status.AUTH_ERROR) {
      this.listSasl();
    } else if (response.header.opcode == protocol.status.AUTH_CONTINUE) {
...
Server.prototype.sock = function (go) {
  ...
  if (!self._socket) {
    self._socket = net.connect(this.port, this.host, function () {
      ...
      // do sasl authentication at start up
